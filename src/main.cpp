...
unsigned int nStakeTargetSpacingOLD = 45; // DIFF: 15-minute block spacing
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        if ((!txout.IsEmpty()) && txout.nValue < MIN_TXOUT_AMOUNT_OLD)
...
static const int64 nTargetTimespanOLD = 86375;  // 691 * 125 = 86375 @ 691 blocks
static const int64 nTargetSpacingWorkMaxOLD = 3 * nStakeTargetSpacingOLD; // Ecocoin @ 2.25 minutes (125s)
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
 ...
 /*            if(fTestNet || (pindexLast->nHeight + 1 > 15000))
	...
	{
		nTargetSpacing = fProofOfStake? nStakeTargetSpacingOLD : min(nTargetSpacingWorkMaxOLD, (int64) nStakeTargetSpacingOLD * (1 + pindexLast->nHeight - pindexPrev->nHeight));
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        else
        {
            ...
            if (nTxFee < MIN_TXOUT_AMOUNT_OLD)
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    int test = 0;//Lets begin the test
    ...
    loop
    {
        ...
        CDataStream::iterator pstart2 = search(vRecv.begin(), vRecv.end(), BEGIN(pchMessageStart2), END(pchMessageStart2));
        ...
        //printf("\n\nPROCESSMESSAGE SKIPPED %s BYTES\n\n", (pstart - vRecv.begin()).c_str());
        vRecv.erase(vRecv.begin(), pstart);
        } else {
        if (pstart2 - vRecv.begin() > 0)
        //printf("\n\nPROCESSMESSAGE SKIPPED %s BYTES\n\n", (pstart2 - vRecv.begin()).c_str());
